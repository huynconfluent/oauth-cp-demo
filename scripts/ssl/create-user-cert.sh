...
# OPENSSL_BIN=openssl WORK_DIR=component GEN_DIR=files ./create-user-cert.sh <component_name> <base_subject> <conf> <issuer_key> <issuer_cert> <fullchain_ca_cert> <subject_alt_entries>
# OPENSSL_BIN=openssl WORK_DIR=component GEN_DIR=files ./create-user-cert.sh kafka "/C=US/ST=CA/O=Confluent Demo/OU=Global Technical Support" ./configs/component.cnf ../generated/intermediateca/private/intermediate_3.key ../generated/intermediateca/certs/intermediate-signed_3.pem "../generated/intermediateca/certs/fullchain.pem" "DNS:localhost,DNS:kafka-1.confluentdemo.io"
...
PRIVATE_COMPONENT_KEY_PATH=$WORK_DIR/private
COMPONENT_CSR_PATH=$WORK_DIR/csr
PUBLIC_COMPONENT_CERT_PATH=$WORK_DIR/certs
ISSUER_KEY=$4
ISSUER_CERT=$5
CA_CHAIN=$6
if [ ! -z $7 ]; then
...
$(dirname "${BASH_SOURCE[0]}")/create-keystore.sh $COMPONENT_NAME $PRIVATE_COMPONENT_KEY_PATH/$COMPONENT_NAME.key $PUBLIC_COMPONENT_CERT_PATH/$COMPONENT_NAME-fullchain.pem topsecret "$GEN_DIR"
